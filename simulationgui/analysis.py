...
class SelectionMethod:
    def trained_model(self, data, dependent_var):
        raise NotImplementedError("users must define trained_model to use this base class'")


class LassoCVMSE(SelectionMethod):
    def __init__(self):
        self.name = "LassoCV"

    def trained_model(self, data, dependent_var):
        return lasso_cv_mse(data, dependent_var, cv=10, alphas=np.arange(0.01, 1.0, 0.05), stddev=0)


class LassoCVMSEStd(SelectionMethod):
    def __init__(self):
        self.name = "LassoCVStd"

    def trained_model(self, data, dependent_var):
        return lasso_cv_mse(data, dependent_var, stddev=1.0)


class LassoAlpha(SelectionMethod):
...
class LassoBIC(SelectionMethod):
    def __init__(self):
        self.name = "LassoBIC"
...
class RandomForest(SelectionMethod):
    def __init__(self):
...
def select_variables(model):
    ...
    Arguments:
        model: Some already fit regression model, like Lasso or LinearRegression
    Example:
...
def predict_worker_func(method, true_model_text, data_model, trials, num_samples):
    model_generator = method.trained_model
    sum_sq_err = 0.0
...
def worker_func(method, true_model_text, data_model, trials, num_samples, true_variables):
    ...
    forced_1_ind = set(map(lambda s: s+1, data_model.forced_variables_ind))

    for trialnum in range(trials):
        print("Trial %d, num_samples: %d, method: %s" % (trialnum, num_samples, method.name))
        sampled_data = get_distribution_samples(data_model, num_samples, true_model_text)

        model = model_generator(sampled_data, data_model.dependent_var)

        chosen_variables = select_variables(model).union(forced_1_ind)
...
def subset_accuracy(variables, data_model, true_model_text, sample_range, trials, subset_metrics, subset_methods, predict_methods, error_types):
    ...
    for method in predict_methods:
        ...
        output_data[(method.name, 'prediction_mse')] = mse
    ...
    for method in subset_methods:
        ...
        output_data[(method.name, 'perfectly_chosen')] = arr_perfectly_chosen
        output_data[(method.name, 'predictors_missed')] = arr_predictors_missed
        output_data[(method.name, 'false_predictors_chosen')] = arr_predictors_missed
        output_data[(method.name, 'symm_diff')] = arr_symm_diff
        output_data[(method.name, 'symm_diff_2')] = arr_symm_diff_2
        output_data[(method.name, 'ave_symm_diff')] = arr_ave_symm_diff
        output_data[(method.name, 'prediction_mse')] = mse
    pool.close()
    return output_data
